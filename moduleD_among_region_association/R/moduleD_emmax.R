...
source("moduleD_among_region_association/R/emmax.R")        # PK's emmax()/emma.REMLE() (Li, Kemppainen, Rastas & Merila); copied verbatim
source("moduleD_among_region_association/R/moduleD_paralogy.R")   # cross-chromosome duplication filter (shared with the Ohta arm)
set.seed(1)
...
OHTA       <- "moduleD_among_region_association/data/moduleD_ohta.rds"                     # for negative/positive-R_st focal picks
...
saveRDS(list(results = res_list, hits = hits, focal = focal, het_of = het_of,
        ...
        "moduleD_among_region_association/data/moduleD_emmax.rds")
...
pdf("moduleD_among_region_association/Figures/moduleD_emmax_manhattans.pdf", width = 9, height = 3.2)
