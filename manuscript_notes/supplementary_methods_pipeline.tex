...
analysis throughout is the LD block rather than the marker; quantities are
measured per block, and statistical tests treat blocks, not markers, as
...
depend on block size is expressed as a rate rather than a count.
...
in local LD, and is used below to identify the regions where Stage~1 clustering
is most likely to have been conservative.
...
block gives an LD-pruned marker set. Collapsing each block to a single consensus
dosage gives an expected multi-locus genotype (eMLG) per block. Both are
produced here from a single clustering pass, so the two representations always
refer to the same underlying units.

\subsection{Stage 1: local LD blocks}
...
a genuine single block wider than the window is fragmented into several
...
severs real blocks exactly at the selection boundary, whereas selecting whole
clusters leaves genuine blocks intact.
...
second criterion captures substantial clusters whose local LD support is
moderate and which would otherwise pass through unmerged. Flagged clusters were
...
$\rsq \ge 0.2$, since the fidelity criterion alone can be satisfied by a merge
of essentially uncorrelated markers. Because merging proceeds only while both
conditions hold, this stage cannot over-merge regardless of how permissively
...
instead of clipping a genuine block to a distance calibrated on typical
chromosomes.
...
aggregated blocks, and genome-wide summaries are simple tallies over units.
...
recombination, block size, climate association and the direction of sorting.
...
\subsection{Testing at the level of blocks}
The same statistic was applied to the eMLG consensus genotypes so that each LD
block contributes one test rather than one test per constituent marker. Stored
consensus genotypes cover blocks of at least five markers; for the remaining
blocks containing a sorted marker, consensus genotypes were constructed by the
identical procedure, so that every block is tested at whatever resolution its
size permits and none is represented only by its individual markers.

Testing a block requires parental as well as hybrid consensus genotypes, since
...
The diagnostic index of a block was taken as the maximum across its member
markers, so that a block is excluded by a DI criterion only if none of its
members is ancestry-informative. Using the representative marker alone would
exclude blocks whose informative markers lie elsewhere in the block.

\subsection{Distinguishing absence of signal from dilution}
A block may contain a marker that sorts individually and yet not sort once its
...
block whose consensus is faithful and genuinely does not sort, versus a
heterogeneous block in which averaging across dissimilar members dilutes a real
...
than decorative. Adjacent markers within a low-recombination block report the
...
When relating recombination to sorting, block size is a consequence of both
...
(Section~\ref{sec:climate}), block size is a confounder. It independently
predicts DI genome-wide, and it is also required by any criterion that counts
significant members within a block. It is therefore adjusted for in those models.
...
recombination rate, pooled parental allele frequency and log block size, fitted
across units. Any recombination term here is an association: under neutrality a
non-recombining autosomal block does not experience stronger drift than its
...
\subsection{Association strength per block}
Each block's climate association was summarised as its \emph{significance
...
eligibility depend on block size. A block of nine markers would need more than
half of its members to be individually significant in order to reach a count of
five, whereas a block of ninety needs only a small fraction; the same nominal
criterion is therefore far stricter for small blocks. This cannot be repaired by
conditioning on block size, because the resulting eligibility is a hard,
...
construction, and block size is retained as a covariate for its independent
effects.

Because a rate is estimated imprecisely when a block contains few markers,
results are reported across nested size strata rather than pooled: all blocks,
...
outcome, the quantity modelled is the proportion of a block's members that are
ancestry-informative, with each block contributing one observation. A
...
analysis: it treats markers within a block as independent trials, whereas they
...
sorting classification was tested in the same way, with the block's
...
Sorting & near-fixation tolerance & 0.15 \\
 ...
 & block diagnostic index & maximum across members \\
