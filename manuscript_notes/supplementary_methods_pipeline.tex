...
The two floors also bound the correlation that can survive between distinct
clusters, and thereby the independence of the resulting units. A pair is left
unmerged either because it falls below the correlation floor, in which case its
members are already close to independent, or because merging would breach the
fidelity floor, which is where a large and coherent cluster resists absorbing further
markers; in the latter case the residual correlation between neighbouring clusters is
limited by how much the consensus can tolerate. Measured after the fact on the
merged units---and after correcting for relatedness, so that shared population
structure does not inflate it---the largest such residual is of order $r^2 \approx
0.3$. This is comparable to the fixed threshold ($r^2 = 0.2$) that conventional LD
pruning applies uniformly, but is obtained on a relatedness-corrected scale between
consensus genotypes rather than as a raw correlation between individual markers. Any
residual correlation among clusters is moreover local, confined to neighbours within
a short genetic distance; where independent observations are required across long
distances---as in the two-locus screens for epistasis, where correlated neighbouring
clusters would otherwise represent a single event several times---a genetic-map
distance threshold removes it directly.
...
repeatedly. A pair was treated as unlinked if the two clusters lay on different
chromosomes or on the same chromosome more than a threshold genetic distance apart;
the remaining same-chromosome pairs, within that distance, were retained not as
tests but as a close-linkage reference against which the unlinked pairs are read.
The restriction is what makes the test specific: linked clusters can carry
disequilibrium through recombinational linkage irrespective of any interaction,
whereas sufficiently distant clusters cannot, so an association there must arise
from correlated sorting rather than from proximity.

The threshold was set on the genetic rather than the physical map, and chosen from
the age of the hybrid populations. Admixture disequilibrium between two loci decays
as $(1-c)^{t}$ in the recombination fraction $c$, independent of physical distance,
so the distance beyond which founding disequilibrium has dissipated is a property of
map distance and generation number alone. At the $\sim\!50$ generations implied for
these populations, $10$~cM corresponds to about $99\%$ decay (Haldane $c \approx
0.088$), and the mean ancestry-tract length, $\sim\!1/t$ Morgans or about $2$~cM,
makes $10$~cM roughly five tract lengths---so clusters that far apart segregate on
effectively independent ancestry tracts. The threshold is also supported directly by
the data: among-population disequilibrium between same-chromosome clusters decays to
the inter-chromosomal baseline by about $10$--$15$~cM. Because these are
high-recombination chromosomes ($\sim\!19$~cM/Mb), $10$~cM corresponds to roughly
half a megabase.
...
Intrinsic LD & unit & eMLG consensus (hard-called) \\
 & unlinked definition & different chr, or same chr $> 10$ cM \\
