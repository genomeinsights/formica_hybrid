...
populations (Fig.~X, panel A). Thus, the main feature of the data was not
...
distribution (Spearman \(\rho=0.31\); Fig.~X, panel B). In a logistic model
...
(Fig.~X, panel C). The excess marker-level signal in low recombination
...
similar conclusions from 10-cM blocks (Fig.~X, panel D).
...
\centering
\begin{tikzpicture}[
  panel/.style={draw=Accent, thick, rounded corners=2pt, fill=LightBlue,
    minimum width=72mm, minimum height=43mm, text width=64mm,
    align=left, inner sep=5mm},
  title/.style={font=\bfseries\color{Accent}}
]
\node[panel] (a) {\textbf{A. Directional sorting}\\[2mm]
\textcolor{Pol}{17.50\% \Fpo{}} \quad
\textcolor{Aqu}{14.40\% \Faq{}}\\
0.14\% bidirectional; 67.96\% unsorted\\[2mm]
\footnotesize Stacked proportions among parent-polymorphic SNPs.};
\node[panel, right=8mm of a] (b) {\textbf{B. Direction reverses with DI}\\[2mm]
\(\Pr(\Faq{}\mid\mathrm{unidirectional})\) rises from
\(\sim0.15\) to \(\sim0.74\) across DI deciles.\\[2mm]
\footnotesize Line and uncertainty across deciles; parity at 0.5.};
\node[panel, below=8mm of a] (c) {\textbf{C. LD reveals pseudoreplication}\\[2mm]
Lowest recombination decile:\\
17\% sorted SNPs versus 6\% sorted units.\\[2mm]
\footnotesize SNP and LD-reduced-unit estimates across recombination deciles.};
\node[panel, right=8mm of c] (d) {\textbf{D. Climate effects are not robust}\\[2mm]
Nominal diagnostic effects: \(+4.5\) and \(+3.7\) pp,\\
but block intervals include zero.\\
Sorting: OR \(0.99\) and \(1.03\).\\[2mm]
\footnotesize Model-based and block-robust intervals for PC1/PC2.};
\end{tikzpicture}
\caption{\textbf{Wireframe for the main ancestry-sorting figure.} The panels
progress from the phenomenon (A), to its principal predictor (B), to the effect
of genomic linkage on its apparent abundance (C), and finally to its
