...
## Table G — objects `R/15_candidate_import.R` through `R/22_candidate_synthesis.R`
## (the candidate-locus population-frequency/profile-similarity follow-up,
## 2026-09-14) read from `module_manuscript_rho05`

A **separate, later analysis universe** from Table A/B above (18,361-unit
Stage-1-direct BayPass-scan candidates, 19 populations Åland-excluded — NOT
the 20,807-unit DI25 rho05 primary unit set). Near-direct transcription of
`data/followup/15_manifest.rds`, generated by `R/15_candidate_import.R` and
re-printed by `R/22_candidate_synthesis.R`; treat this table as a snapshot,
refresh from the manifest on demand, same convention as Tables A–D above.

| path | role | status |
|---|---|---|
| `module_manuscript_rho05/data/moduleB_stage1_units_bestsnp.rds` | Stage-1-direct unit universe (group_id/Chr/n_loci) + best-SNP genotype dosage (raw, unoriented, hybrids-only) | authoritative, currently read |
| `module_manuscript_rho05/data/moduleB_stage1_S1units_null.rds` | PC1/PC2 raw BF(dB) + null-calibrated floor-survivor status | authoritative, currently read |
| `module_manuscript_rho05/data/moduleB_stage1_bio_winter_null.rds` | bio_winter raw BF(dB) + null-calibrated floor-survivor status | authoritative, currently read |
| `module_manuscript_rho05/data/moduleB_stage1_mitoC2_null.rds` | mitoC2 raw C2/log10p + null-calibrated floor-survivor status (`floor3 = log10p>=3 & k3==0`) | authoritative, currently read |
| `module_manuscript_rho05/data/moduleB_stage1_mitoC2_regions.rds` | STALE — pre-null-calibration mitoC2 crossings (11 raw, mtime predates the current `floor3` calibration, which correctly finds 0 floor survivors) | **excluded** — confirmed stale, superseded by `moduleB_stage1_mitoC2_null.rds$floor3` |
| `module_manuscript_rho05/data/moduleC_stage1_annotations.rds` | recomb (cM/Mb)/Chr/Pos/DI (full-genome ungated vintage)/prop_fixed/uni_score/eBF1/eBF2 per unit | authoritative, currently read |
| `module_manuscript_rho05/data/moduleA_stage1_cluster_sorting.rds` (+`_tau05`/`_tau08`) | sort_class @ tau=0.6 (primary)/0.5/0.8 + `uni_score` (orientation ground truth, `orient="parents"` convention) | authoritative, currently read |
| `data/hybrids_and_parents_maf005.Rdata` | sample metadata (Population/Sample_ID) + parental genotype dosage for orientation + parental MAF | authoritative, shared — same object Table A uses |
| `data/liftoff_Frufa_DTOL_PR/bdmi_candidates.cutoff_13_003.liftoff.Frufa_DTOL_PR.bed` | BDMI candidate-region overlap, cutoff index 13 (documented default, adjustable — `module_di25`'s own "good coverage" single-cutoff precedent) | authoritative, currently read |
| `module0_ld_pruning_rho05/data/eMLG_5loci_0025_cM05_rho05_bestsnp.rds` | genome-wide structure-PCA reference (17,509-unit large-cluster subset of the 661,386-unit canonical panel — same object as Table D, different subset used) | authoritative, currently read |

**Population-order/set warning specific to this table**: `module_manuscript_rho05`'s
own Stage-1 metadata orders the 19 hybrid populations (Åland excluded)
alphabetically; this module's own Table A/B Fmat uses a non-alphabetical
first-appearance order and includes Åland (20 populations). Table G's
scripts re-key every join on population NAME, never position, and exclude
Åland throughout — do not assume row-order compatibility between Table A/B
objects and Table G's outputs.
