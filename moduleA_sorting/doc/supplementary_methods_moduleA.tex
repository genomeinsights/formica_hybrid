...
near-fixed. Here \(\phi=0.85\) is the frequency the fixed-for (major) allele must
reach, equivalently the requirement that the opposing allele fall to at most
\(1-\phi=0.15\). The same threshold was used throughout because the attainable
...
numbers near-fixed toward \Faq{} and \Fpo{}, respectively. Sorting magnitude was
the total proportion near-fixed,
\begin{equation}
 p_{\mathrm{fixed}}=
 \frac{n_{\mathrm{aqu}}+n_{\mathrm{pol}}}{n_{\mathrm{obs}}}.
\end{equation}
A unit passed the sorting-magnitude gate when
\(p_{\mathrm{fixed}}\geq\tau=0.6\). Among units passing this gate, directional
consistency was assessed by an exact two-sided binomial test of
\(n_{\mathrm{aqu}}\) successes in
\(n_{\mathrm{fixed}}=n_{\mathrm{aqu}}+n_{\mathrm{pol}}\) trials under a
random-direction null probability of 0.5. A significant imbalance
(\(P<0.05\)) defined unidirectional sorting, with the parental direction set by
the larger count. A non-significant split was classified as bidirectional when
\(n_{\mathrm{fixed}}\) was sufficient for the test to reject an all-one-parent
outcome under the null; otherwise it was labelled ambiguous. Units failing the
magnitude gate were unsorted. Exchanging parental labels changes the direction
but not sorting magnitude or class.

The two thresholds were treated separately. The near-fixation floor \(\phi\) is
the per-population convention; the sorting threshold
...
monotonically with either threshold, whereas the diagnostic-index direction
...
and separates the prevalence of near-fixation from evidence that its parental
direction is repeated more consistently than expected under random direction.
...
selection. Coefficient sensitivity will be regenerated with the same binomial
direction rule before it is included in the supplementary results.
...
\texttt{parallelism\_stats.R} and \texttt{eMLG\_parallelism.R}. Ohta-statistic
functions used elsewhere in the project are supplied by \texttt{LDscnR}.
