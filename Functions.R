...
plot_ld_outlier_circos <- function(
  ...
  if (!is.null(manhattan_color_col)) {
    manhattan_color_col <- as.character(manhattan_color_col)
    if (!manhattan_color_col %in% names(map_manh)) {
      stop("manhattan_color_col is not a column in map_manh: ", manhattan_color_col)
    }
  }
    ...
    for (stat in manhattan_stat_col) {
      ...
      if (is.null(manhattan_color_col)) {
        ...
        dd <- map_manh[, .(
          ...
          OR_id = as.character(OR_id),
          raw_stat = get(stat),
          plot_col = as.character(get(manhattan_color_col))
        )]
      }
    ...
    for (stat in names(manh_list)) {
      ...
      circos.trackPlotRegion(
        ...
        panel.fun = function(x, y) {
          ...
          if (nrow(dd) > 0) {
            if (!is.null(manhattan_color_col) && "plot_col" %in% names(dd)) {
              cols <- dd$plot_col
            } else {
