...
"## Data provenance",
"",
"- **Scope (AUDIT FIX, Issue 4): this is the Stage-1 LARGE-CLUSTER scan, not an unrestricted scan of every LD-reduced unit.** The Omega/GRM basis is estimated from all 698,251 Stage-1 clusters (including singletons), but the tested/calibrated unit universe here is the 18,361 clusters with >=5 SNPs (best-SNP represented) -- 2.6% of all Stage-1 clusters. Isolated SNPs and clusters with fewer than 5 markers were NOT tested here; biological conclusions below are scoped to large, well-supported LD clusters, not to the genome's full LD-reduced unit set (that full set, 661,386 units, IS used elsewhere in this pipeline -- see the Fst-vs-DI analysis, `di25_fst_vs_di_rho05.R` -- but not for this climate calibration).",
