## =========================================================
## module_manuscript_rho05 -- sorting vs recombination, swept over tau (eMLG side, rho05)
## =========================================================
## Mirrors module_di25/R_legacy/di25_recomb_tau_sweep.R exactly, but consumes
## the min_r2_rho = 0.5 eMLG sorting/clustering (module_di25_rho05/data/) for
## the eMLG level. The SNP level does not depend on clustering at all and is
## reused unchanged from module_di25/data/di25_sorting_snp.rds.
##
## Feeds Fig 3 (main text) multipanel's recombination panel and the
## standalone Fig [sorting_recomb] under the rho05 methodology.
##
## Run from the repo root:  Rscript module_manuscript_rho05/R/di25_recomb_tau_sweep_rho05.R
## =========================================================
...
N_MIN    <- 100L
SEED     <- 1
OUTDIR   <- "module_manuscript_rho05/data"; FIGDIR <- "module_manuscript_rho05/Figures"
...
s <- as.data.table(readRDS("module_di25/data/di25_sorting_snp.rds"))    # reused unchanged (SNP level)
...
saveRDS(list(deciles = dec, sweep = sweep, r_lo = r_lo,
        ...
        file.path(OUTDIR, "di25_recomb_tau_sweep_rho05.rds"))

cat("\n===== sorting vs recombination, tau sweep (eMLG, min_r2_rho=0.5, reliable range recomb>=",
    round(r_lo,1), "cM/Mb) =====\n", sep="")
print(sweep)
...
pA <- ggplot(de, aes(rbin)) +
  ...
  labs(x = "recombination decile (median cM/Mb)",
       title = "Sorting vs recombination across tau (min_r2_rho = 0.5)",
       subtitle = "grey = low-n deciles (not reliably estimable)") +
...
pB <- ggplot(sweep, aes(factor(tau), coef_recomb)) +
  ...
  theme_bw(base_size = 10)
ggsave(file.path(FIGDIR, "di25_recomb_tau_sweep_rho05.png"), pA + pB + plot_layout(widths = c(2, 1)),
       width = 13, height = 5.2, dpi = 200)
cat("\n[di25-recomb-sweep-rho05] wrote", file.path(FIGDIR, "di25_recomb_tau_sweep_rho05.png"), "\n")
