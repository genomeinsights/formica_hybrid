...
\textbf{This is fully compatible with, and plausibly explains, the original canonical
BayPass climate scan's null-calibrated result} (Stage-2 eMLG-level scan, 32{,}854
clusters, reported in the main manuscript -- the Stage-1-direct scan's own structured null,
Sections~1--4 of this document notwithstanding, is still running and not yet available for
this comparison). A moderate alignment between climate and ancestry can cause loci that
strongly differentiate the underlying population groups to acquire large \emph{raw} Bayes
factors, simply because those loci track the same population split the climate variable
happens to track. But $\Omega$-structured null covariates -- random vectors sharing only
the populations' covariance structure, with no causal link to climate at all -- frequently
produce similarly-aligned, similarly-sized apparent outliers. Consequently the observed
candidate counts were not exceptional against that null:
\begin{itemize}
  \item No PC1 cluster exceeded all 10{,}000 structured-null covariates.
  \item Four PC2 clusters did, but $\approx3.29$ were expected by chance alone, giving an
    estimated set-level FDR of $\approx0.82$.
\end{itemize}
The structured null is, in effect, saying: \emph{these apparent climate associations are
approximately what is expected when a covariate happens to align with population history.}
...
$\Omega$-structured null covariates. The conclusion is \emph{not} that climate definitely
...
population structure already predicts. This document should be read as a supplementary,
exploratory analysis, not a confirmed finding for the main text.
