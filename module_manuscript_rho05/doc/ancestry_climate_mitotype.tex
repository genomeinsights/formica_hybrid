...
\textbf{Second erratum (2026-09-12, systematic pipeline audit).} A further implementation
bug was found in the code above: the reduced $\Omega_{17}$ matrix was built in
\texttt{unique()}'s first-appearance population order, while the phenotype table
\texttt{dt17} is sorted alphabetically (\texttt{setorder}) -- two genuinely different
orderings, verified by direct comparison. Every $\Omega$-null draw was therefore compared
against the wrong lineage unit's phenotype, with no name-based alignment to catch it.
\textbf{Fix:} $\Omega_{17}$ is now explicitly reordered to \texttt{dt17\$unit\_id} via
\texttt{match()}, with a hard rownames/colnames assertion. All six corrected $\Omega$-null
$p$-values below now match an independent colleague's cross-check exactly (ancestry--PC1
$0.074$, ancestry--PC2 $0.190$, ancestry--bio6 $0.142$, ancestry--bio11 $0.292$,
ancestry--mitotype $0.050$, partial PC1--ancestry$\mid$mitotype $0.141$) -- strong evidence
the fix is correct. Raw correlations and block-permutation results do not depend on
$\Omega$'s row order and are, as expected, unchanged. This same audit also corrected a
covariate-standardization bug in the Stage-1-direct BayPass scan referenced in Section~5,
materially changing that section's floor-survivor counts; see AUDIT\_FIXES.md in this
module for the full list of fixes.
...
\midrule
PC1   & $-0.481$ & $0.074$ & $0.051$ & $0.164$ \\
PC2   & $+0.381$ & $0.190$ & $0.123$ & $0.164$ \\
bio6  & $+0.404$ & $0.142$ & $0.110$ & $0.164$ \\
bio11 & $+0.287$ & $0.292$ & $0.267$ & $0.267$ \\
\bottomrule
\end{tabular}
...
right at the boundary) -- but the two null designs disagree (\smash{$\Omega$-null $p=0.074$}
...
\midrule
ancestry & $+0.490$ & $0.050$ & $0.048$ & $0.242$ \\
PC1      & $-0.299$ & -- & $0.254$ & $0.424$ \\
...
($p\approx0.048$--$0.050$ uncorrected) but \textbf{this does not survive correction for the
5-comparison family} ($p_{\mathrm{BH}}=0.242$). No climate variable shows any evidence of a
...
\toprule
 & $r$ (17 units) & Notes \\
\midrule
Raw cor(PC1, ancestry)                     & $-0.481$ & $\Omega$-null $p=0.074$; block-perm.\ $p=0.051$ \\
Partial cor(PC1, ancestry $\mid$ mitotype) & $-0.402$ & $\Omega$-null $p=0.141$; block-perm.\ $p=0.124$ \\
\bottomrule
\end{tabular}
...
distinguishable from the population-structure null under either test ($p=0.124$--$0.141$).
...
($p=0.074$), nominal under the corrected lineage permutation ($p\approx0.051$), and
non-significant after controlling for mitotype ($p=0.124$--$0.141$; Section~4). PC2 is
...
\midrule
PC1 floor survivors & 0 & 1 & 1.84 (Stage-1) / 3.29 (Stage-2) \\
PC2 floor survivors & 4 & 2 & 1.84 (Stage-1) / 3.29 (Stage-2) \\
bio\_winter floor survivors & -- & 10 & 1.84 \\
mitoC2 floor survivors & -- & 0 & 1.84 \\
Set-level FDR & $\approx0.82$ (PC2) & $1.84$ (PC1) / $0.92$ (PC2) / $\mathbf{0.18}$ (bio\_winter) & -- \\
\bottomrule
\end{tabular}
\caption{Floor-survivor candidate counts (BF(dB)$\geq$15, or $-\log_{10}(p)\geq3$ for
mitoC2, and beating all 10{,}000 $\Omega$-structured null covariates), AUDIT-CORRECTED
2026-09-12 (covariate standardization fix -- see AUDIT\_FIXES.md, Issue 1). bio\_winter is
the mean of standardized bio6/bio11 (Issue 7 decision: one combined winter-temperature axis,
not two near-duplicate tests). ``Expected by chance'' is $N_{\mathrm{tested}}/(n_{\mathrm{sim}}+1)$.}
\end{table}

\textbf{These counts changed materially after an audit fix.} An earlier version of this
comparison (before the observed PC1/PC2/bio6/bio11 covariates were corrected to match the
null covariates' standardization) reported 3 PC1 and 3 PC2 floor survivors for the
Stage-1-direct scan, read as a ``Goldilocks'' recovery of signal the Stage-2-merged scan had
diluted. That reading does \emph{not} survive the correction: PC1 now has just 1 survivor
(BF(dB)$=28.5$) -- \emph{fewer} than the null's own expectation (1.84), i.e.\ indistinguishable
from, if anything below, chance -- and PC2's 2 survivors (FDR$\approx0.92$) remain similarly
unremarkable. Neither scan, PC1 nor PC2, is a defensible discovery set.
\textbf{bio\_winter is the exception:} 10 of 18{,}361 units survive the floor test, a
substantially lower set-level FDR ($\approx0.18$) than either PC1 or PC2 achieve on either
scan -- the one climate result in this document that looks like more than population
structure alone would produce, and worth following up on its own terms (not explored further
here). mitoC2 shows the opposite: 0 of 11 raw threshold crossings survive the floor test,
consistent with Section~3's finding that mitotype has no detectable relationship with any
measured climate variable. The structured null is, in effect, saying: \emph{these apparent
PC1/PC2 climate associations are approximately what is expected when a covariate happens to
align with population history, regardless of how the markers were reduced to test units --
but bio\_winter's alignment is not.}
...
strength of PC1/PC2 cluster-level BayPass associations exceeded what was obtained under
$\Omega$-structured null covariates, under \emph{either} the Stage-2 or the Stage-1-direct
scan (set-level FDR $\geq0.9$ in both, post audit fix). The one exception is bio\_winter
(FDR$\approx0.18$), which is not explained by the ancestry-confound argument developed here
and is left as an open, unexplored lead. The conclusion for PC1/PC2 is \emph{not} that
climate definitely has no relationship with ancestry -- it is that the data cannot separate
a climate effect from population history, and the BayPass outliers provide no additional
locus-specific evidence beyond that structure. Even a formally significant genome-wide
PC1--ancestry
...
its BF(dB) on PC1, PC2, or bio\_winter -- itself predictable from genomic architecture
(ancestry differentiation, recombination rate, unidirectional ancestry sorting) any more or
less than an $\Omega$-structured pseudo-covariate with no causal link to climate would show
by chance? For each of PC1, PC2, bio\_winter, and 10{,}000 null covariates (structured draws
through $\Omega$'s own eigendecomposition, sharing the populations' covariance structure but
carrying no climate information), the genome-wide BF vector is reduced to three statistics --
Spearman correlation with the per-unit Diagnostic Index (DI), Spearman correlation with
recombination rate, and a directional-sorting contrast (mean BF percentile of
unidirectionally-sorted vs.\ non-directional units, among differentiated units only) -- and
each observed statistic is compared to its own 10{,}000-value null distribution. PC1, PC2,
and bio\_winter are tested against the \emph{identical} null distribution (the null depends
only on $\Omega$/architecture, not on which real covariate is observed, and this identity was
checked directly rather than assumed), so the nine resulting tests (3 axes $\times$ 3
architecture measures) are Benjamini-Hochberg corrected as one family.

The same question was also asked of mitoC2, the BayPass C2 population-contrast statistic for
the \Faq{}-/\Fpo{}-like mitotype split (Section~3). mitoC2 is not a continuous-covariate
regression, so the PC1/PC2/bio\_winter null does not apply: it uses its own dedicated null of
10{,}000 population-level $+1/-1$ partitions, each matching the real split's group sizes
(7 vs.\ 12 populations) and rank-thresholded from the same $\Omega$-eigenvector draws, then
re-run through BayPass in contrast mode. Because this is a structurally different
null-generating process, mitoC2's three tests form their own Benjamini-Hochberg family, not
pooled with the continuous-covariate family above.
...
\midrule
DI (Spearman $\rho$)            & PC1         & $+0.100$ & $+0.029$ & $[-0.018, 0.072]$ & $0.005$ \\
DI (Spearman $\rho$)            & PC2         & $-0.131$ & $+0.029$ & $[-0.018, 0.072]$ & $<0.001$ \\
DI (Spearman $\rho$)            & bio\_winter & $-0.055$ & $+0.029$ & $[-0.018, 0.072]$ & $0.001$ \\
Recombination (Spearman $\rho$) & PC1         & $-0.099$ & $+0.065$ & $[-0.013, 0.144]$ & $<0.001$ \\
Recombination (Spearman $\rho$) & PC2         & $+0.020$ & $+0.065$ & $[-0.013, 0.144]$ & $0.269$ \\
Recombination (Spearman $\rho$) & bio\_winter & $+0.142$ & $+0.065$ & $[-0.013, 0.144]$ & $0.063$ \\
Sorting (differentiated only)   & PC1         & $+0.036$ & $+0.123$ & $[0.078, 0.168]$  & $0.001$ \\
Sorting (differentiated only)   & PC2         & $+0.019$ & $+0.123$ & $[0.078, 0.168]$  & $0.001$ \\
Sorting (differentiated only)   & bio\_winter & $+0.036$ & $+0.123$ & $[0.078, 0.168]$  & $0.001$ \\
\midrule
\multicolumn{6}{l}{\textit{mitoC2 -- dedicated contrast-mode null, own BH family (not pooled with the rows above)}} \\
\midrule
DI (Spearman $\rho$)            & mitoC2 & $+0.015$ & $+0.021$ & $[-0.037, 0.069]$ & $0.963$ \\
Recombination (Spearman $\rho$) & mitoC2 & $+0.006$ & $+0.008$ & $[-0.082, 0.114]$ & $0.963$ \\
Sorting (differentiated only)   & mitoC2 & $+0.037$ & $+0.016$ & $[-0.030, 0.072]$ & $0.963$ \\
\bottomrule
\end{tabular}
\caption{Genome-wide architecture calibration, Stage-1-direct universe (18{,}361 units).
``Null median''/``Null 95\%'' describe the same statistic computed on 10{,}000 null
covariates/contrasts -- i.e.\ what a typical covariate or contrast sharing only the
populations' covariance structure, with no climate/mitotype information, would produce.
PC1/PC2/bio\_winter share one null and one nine-test BH family; mitoC2 uses its own
dedicated contrast-mode null and its own three-test BH family (see text).}
\end{table}

\textbf{Finding: PC1, PC2, and bio\_winter all depart from the structured null, but not in a
single consistent direction, and mitoC2 does not depart from its null at all.} DI is
significantly correlated with BF strength on all three continuous axes, but not uniformly:
PC1 is above the null range ($+0.100$), while PC2 ($-0.131$) and bio\_winter ($-0.055$) are
both below it -- so PC1's DI enrichment does not generalize even to the other two axes tested
here, let alone establish a shared ``climate BF tracks ancestry'' mechanism. Recombination
shows a real, negative effect (BF concentrated in low-recombination regions) only for PC1
($\rho=-0.099$); PC2 is indistinguishable from the null ($p_{\mathrm{BH}}=0.269$) and
bio\_winter is a near-miss in the \emph{opposite} direction ($\rho=+0.142$,
$p_{\mathrm{BH}}=0.063$). Directional sorting gives the most consistent -- and least
intuitive -- pattern: on \emph{all three} axes, real climate BF is \emph{less} elevated in
directionally-sorted (vs.\ non-directional) differentiated units than the null baseline
predicts ($0.036$, $0.019$, $0.036$ vs.\ a null median of $0.123$), the opposite of a naive
``sorted regions are more differentiated, so they mechanically inflate any covariate's BF''
argument.

mitoC2's genome-wide landscape, by contrast, is indistinguishable from its own null on all
three measures ($p_{\mathrm{BH}}=0.963$ throughout, the observed value sitting inside the
null 95\% interval on every panel). This is a useful calibration check on the framework
itself, not just a substantive result for mitotype: the DI/recombination/sorting reduction
code, the annotation set, and the unit universe are identical to the PC1/PC2/bio\_winter
tests above, so a covariate landing cleanly inside its null -- reinforced here by an exact,
not Monte-Carlo-tolerance, match between the null reduced for this test and the
independently-computed floor-test exceedance counts of Section~5 -- is evidence that the
shared machinery is not manufacturing spurious enrichment on its own. That makes the
PC1/PC2/bio\_winter departures above more credible as genuine architectural relationships
rather than artifacts of the calibration procedure. mitoC2 is not a designed negative
control, however (there is no a priori guarantee its true genome-wide signal is zero), so
this supports the method's specificity rather than proving it.
...
remains the floor-survivor test's job. PC1/PC2 still return a high set-level FDR
($\geq0.6$) under both scans, while bio\_winter's floor test returns 10 of 18{,}361 units at
FDR$\approx0.18$ (Section~5) despite this section's finding that bio\_winter's
\emph{overall} genome-wide BF landscape trends, if anything, \emph{away} from high-DI and
sorted regions -- a handful of individually exceptional loci coexisting with a bulk
distribution pulling in the opposite direction from what a simple ancestry-confound story
would predict. None of these floor-survivor sets overlap physically: mapping each
covariate's floor-survivor units onto the canonical Stage-2 (rho05) clustering
(\texttt{eMLG\_5loci\_0025\_cM05\_rho05.rds}) gives zero shared Stage-2 groups between any
pair of PC1 (1 region), PC2 (2), mitoC2 (0), and bio\_winter (7) -- even the closest case, a
PC1 and a bio\_winter floor-survivor both on Chr24 only $\sim$53~kb apart, falls into two
distinct Stage-2 clusters (Figure~\ref{fig:region-manhattan-combined}).

\begin{figure}[H]\centering
  \includegraphics[width=0.78\linewidth]{../Figures/moduleB_stage1_snp_manhattan_combined.png}
  \caption{\label{fig:region-manhattan-combined}Stage-1-direct genome-wide scan, all four
  covariates. Every genome-wide SNP is plotted at its own full-SNP BayPass value -- BF(dB)
  for PC1, PC2, and bio\_winter (continuous-covariate regression); the C2 contrast statistic
  $-\log_{10}(p)$ for mitoC2 (population contrast). Dashed line: the raw threshold
  (BF(dB)$\geq15$, or $-\log_{10}(p)\geq3$ for mitoC2). Grey (alternating shade by
  chromosome): no Stage-1 cluster in this canonical Stage-2 (rho05) region raw-crossed the
  threshold. Coloured (one colour per region, recycled): every Stage-2 (rho05) region
  containing $\geq1$ raw threshold crossing -- most of these are raw crossings only, not
  validated candidates. Arrowed and labelled (Stage-2 group\_id): the subset of coloured
  regions that additionally contain a floor-survivor Stage-1 unit (beats all 10{,}000
  $\Omega$-structured null draws), with the label coloured to match its region -- PC1: 45 raw
  crossings $\rightarrow$ 34 regions coloured, 1 floor survivor $\rightarrow$ 1 region
  arrowed (F237397); PC2: 65 raw crossings $\rightarrow$ 42 regions coloured, 2 floor
  survivors $\rightarrow$ 2 regions arrowed (F42664, F53302); mitoC2: 11 raw crossings
  $\rightarrow$ 11 regions coloured, 0 floor survivors, no region arrowed; bio\_winter: 113
  raw crossings $\rightarrow$ 71 regions coloured, 10 floor survivors $\rightarrow$ 7 regions
  arrowed (F24630, F27359, F45674, F183260 [4 Stage-1 clusters], F190105, F191253, F238394).
  No arrowed region is shared between any pair of covariates (see text).}
\end{figure}

What Section~6 adds is a more granular picture: the Stage-1-direct climate-BF
landscape is \emph{not} architecture-agnostic for any of the three continuous axes, but the
departures point in different, and for sorting a uniformly unexpected, direction -- so
\emph{which} architectural feature aligns with which axis, and whether that alignment
reflects the same ancestry confound discussed in Sections~1--4 or a distinct mechanism, is
not resolved here and would need its own follow-up (e.g.\ relating each axis's
DI/recombination/sorting correlation directly to that axis's own ancestry-confound
coefficient from \texttt{moduleB\_ancestry\_confound.R}).

\textbf{Independent cross-check using BayPass's own outlier-region method.} A separate module
(\texttt{module\_localscore\_crosscheck}, own report) re-examines the same four Stage-1-direct
covariates with BayPass's own recommended \emph{local-score} outlier-region method (Fariello
et al.\ 2017; Bonhomme et al.\ 2019), which builds candidate windows directly from each
statistic's spatial profile rather than from the Stage-2 LD-clustering used throughout this
document. Run genome-wide (full-SNP resolution), it independently recovers PC1 and
bio\_winter's shared architecture ($r=+0.89$--$0.90$ genome-wide, matching their near-collinear
raw values and their near-identical significant windows), with PC2 -- a genuinely different
axis -- showing neither. But a direct check of the method's own significance threshold against
the same $\Omega$-structured nulls used throughout this pipeline shows it is not well
calibrated for this dataset: roughly one in five structure-only null draws for the continuous
covariates, and the clear majority for mitoC2's contrast, already produce one or more
``significant'' windows by chance, and a single worst-of-20-random-draws null run through the
actual full-SNP BayPass pipeline produced 104 convincing-looking significant windows from a
covariate with zero true signal by construction -- more than the real mitoC2 covariate itself.
Those null and real mitoC2 windows also overlap more than chance predicts, traced to both
ballooning specifically in the genome's lowest-recombination, highest-LD regions (an artifact
of the method's window-growth mechanism, not of shared true signal). This independent method
therefore reinforces the ancestry-confound reading of PC1/bio\_winter above, but is not used
as a source of candidate loci anywhere in this pipeline.
...
\begin{itemize}
  ...
  \item With 17 independent lineage units and 4-5 tests per family, this analysis is
    ...
    block-permutation $p$-values disagree by up to $\sim$0.02--0.07 in places (e.g.\ PC2:
    $0.190$ vs.\ $0.123$) -- a reminder that ``significant'' here would depend on which
  ...
  \item \textbf{Section~6's genome-wide architecture calibration has mixed-direction results
    ...
    $\Omega$-structured null differently across PC1, PC2, and bio\_winter, and for sorting in
    the opposite direction from a naive confounding argument on all three axes. Adding
    bio\_winter and the mitoC2 calibration check (own dedicated null, no departure on any
    measure) strengthens confidence that these departures are not an artifact of the
    null-covariate machinery, but neither addition explains \emph{why} the three continuous
    axes disagree on direction. Relating each axis's departure directly to that axis's own
    ancestry-confound coefficient (rather than to the coarser population-level ancestry
    summary used in Sections~1--4) would be needed before drawing a mechanistic conclusion
    from it.
\end{itemize}
...
($p=0.074$, and the two nulls disagree), and it did not survive
...
after controlling for mitotype ($r=-0.40$, $p=0.12$--$0.14$). Mitotype was itself nominally
associated with nuclear ancestry ($p\approx0.05$) but not with any measured climate
...
population structure already predicts. A separate, genome-wide calibration (Section~6), extended here to bio\_winter and to mitoC2
as a specificity check, shows the Stage-1-direct climate-BF landscape is not
architecture-agnostic for any of PC1, PC2, or bio\_winter -- seven of nine tests relating BF
strength to Diagnostic Index, recombination, and directional sorting depart from a shared
structured-null baseline -- but the directions differ across the three axes and, for
sorting, run counter to a simple confounding story on all three, so this result sharpens
rather than resolves the population-level picture above. mitoC2, tested the same way against
its own dedicated contrast-mode null, shows no such departure on any of the three measures
($p_{\mathrm{BH}}=0.963$ throughout), reinforcing that the PC1/PC2/bio\_winter departures are
a property of those covariates and not an artifact of the calibration framework itself.
bio\_winter (the combined, standardized bio6/bio11 axis) remains the one climate result that
stands apart from the rest of this ancestry-confound narrative at the individual-locus level:
10 of 18{,}361 Stage-1 units survive the floor test (FDR$\approx0.18$, Section~5), clearly
stronger than PC1 or PC2 achieve on either scan. Section~6 complicates rather than resolves
that lead, however: bio\_winter's \emph{overall} genome-wide BF distribution trends away from
high-DI and directionally-sorted regions, the opposite of what would be expected if its
floor-survivor loci reflected a broad ancestry-tracking signal -- so those 10 units look more
like a distinct, localized signal than the tip of a genome-wide ancestry-confound iceberg,
though this is not established and remains an open lead for future work. This document should
be read as a supplementary, exploratory analysis, not a confirmed finding for the main text.
